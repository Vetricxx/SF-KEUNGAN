...
# --- NERACA SALDO SETELAH PENUTUPAN ---
    ...
    tab1, tab2 = st.tabs(["Input Manual", "Tampilan Neraca Saldo Setelah Penutupan"])
    ...
    with tab1:
        ...
        if st.button("Tambah ke Neraca Saldo Setelah Penutupan"):
            if akun and (debit > 0 or kredit > 0):
                ...
                simpan_session_state()
                st.success(f"Akun '{akun}' berhasil ditambahkan ke Neraca Saldo Setelah Penutupan.")
            else:
    ...
    with tab2:
        if st.session_state.neraca_saldo_setelah_penutupan:
            ...
            if df_edit["Debit"].sum() == df_edit["Kredit"].sum():
                ...
                st.error("⚠️ NSSP Tidak Seimbang")
        else:
